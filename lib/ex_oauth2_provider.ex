defmodule ExOauth2Provider do
  ...
  defp load_resource_owner(
         {:ok, %{resource_owner: %Ecto.Association.NotLoaded{}} = access_token},
         config
       ) do
  ...
  defp load_resource_owner({:ok, access_token}, _config) do
    {:ok, access_token}
  end

  defp load_application_owner({:error, error}, _config), do: {:error, error}

  defp load_application_owner(
         {:ok, %{application: %Ecto.Association.NotLoaded{}} = access_token},
         config
       ) do
    repo = Config.repo(config)
    access_token = repo.preload(access_token, application: [:owner])

    {:ok, access_token}
  end

  defp load_application_owner({:ok, access_token}, _config) do
    {:ok, access_token}
  end
end
